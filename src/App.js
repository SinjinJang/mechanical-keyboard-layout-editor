...
class KeyPlate extends React.Component {
  render() {
    const plateWidth = 2 + 1.75; // TODO: 가장 오른쪽 키의 x + w 계산
    const plateHeight = 2 + 1;  // TODO: 가장 아래쪽 키의 y + h 계산

    const MARGIN_PLATE = 30;
    const style = {
      width: (plateWidth * UNIT_1) + MARGIN_PLATE,
      height: (plateHeight * UNIT_1) + MARGIN_PLATE,
    };
    const styleInner = {
      width: (plateWidth * UNIT_1),
      height: (plateHeight * UNIT_1),
    };
    return (
